...
const h = "@firebase/installations",
    w = new ErrorFactory("installations", "Installations", {
...
async function set(e, t) {
    const n = getKey$1(e),
        r = (await getDbPromise$1()).transaction(v, "readwrite"),
        a = r.objectStore(v),
        o = await a.get(n);
    return (
        await a.put(t, n),
        await r.done,
        (o && o.fid === t.fid) || fidChanged(e, t.fid),
        t
    );
}
async function remove(e) {
    const t = getKey$1(e),
        n = (await getDbPromise$1()).transaction(v, "readwrite");
...
async function update(e, t) {
    ...
    return (
        ...
        await r.done,
        !i || (o && o.fid === i.fid) || fidChanged(e, i.fid),
        i
    );
}
                ...
                if (0 === t.registrationStatus) {
                    ...
                    const n = {
                        ...
                        r = (async function registerInstallation(e, t) {
                            try {
                                const n =
                                    await (async function createInstallationRequest(
                                        {
                                            appConfig: e,
                                        ...
                                        throw await getErrorFromResponse(
                                            "Create Installation",
                                            c
                                        );
                                    })(e, t);
...
function updateInstallationRequest(e) {
    return update(e, (e) => {
        if (!e) throw w.create("installation-not-found");
...
async function generateAuthTokenRequest(
    ...
    const i = { installation: { sdkVersion: "w:0.6.5", appId: e.appId } },
        s = { method: "POST", headers: a, body: JSON.stringify(i) },
...
async function refreshAuthToken(e, t = !1) {
    ...
    const r = await update(e.appConfig, (r) => {
        if (!isEntryRegistered(r)) throw w.create("not-registered");
        const a = r.authToken;
        if (
            !t &&
            (function isAuthTokenValid(e) {
                return (
                    ...
                    !(function isAuthTokenExpired(e) {
                        ...
                        return (
                            t < e.creationTime ||
                            e.creationTime + e.expiresIn < t + 36e5
                        );
                    })(e)
                );
            })(a)
            ...
            return (
                (n = (async function waitUntilAuthTokenRequest(e, t) {
                    ...
                    for (; 1 === n.authToken.requestStatus; )
                        await sleep(100),
                            (n = await updateAuthTokenRequest(e.appConfig));
                    const r = n.authToken;
...
function updateAuthTokenRequest(e) {
    return update(e, (e) => {
        if (!isEntryRegistered(e)) throw w.create("not-registered");
...
const publicFactory = (e) => {
        const t = e.getProvider("app").getImmediate(),
            n = (function extractAppConfig$1(e) {
                if (!e || !e.options)
                    throw getMissingValueError$1("App Configuration");
                if (!e.name) throw getMissingValueError$1("App Name");
                const t = ["projectId", "apiKey", "appId"];
                for (const n of t)
                    if (!e.options[n]) throw getMissingValueError$1(n);
                return {
                    appName: e.name,
...
function getDbPromise() {
    return (
        D ||
            (D = openDB("firebase-messaging-database", 1, {
                upgrade: (e, t) => {
                    if (0 === t) e.createObjectStore(E);
                },
            })),
        D
    );
...
async function dbGet(e) {
    const t = getKey(e),
        n = await getDbPromise(),
        r = await n.transaction(E).objectStore(E).get(t);
    if (r) return r;
    {
                    ...
                    await openDB("fcm_token_details_db", 5, {
                        upgrade: async (n, r, a, o) => {
                            ...
                            if (
                                ...
                                return;
                            const s = o.objectStore("fcm_token_object_Store"),
        ...
        if (t) return await dbSet(e, t), t;
    }
}
async function dbSet(e, t) {
    const n = getKey(e),
        r = (await getDbPromise()).transaction(E, "readwrite");
    return await r.objectStore(E).put(t, n), await r.done, t;
}
function getKey({ appConfig: e }) {
    return e.appId;
}
const C = new ErrorFactory("messaging", "Messaging", {
    "missing-app-config-values":
        'Missing App configuration value: "{$valueName}"',
...
async function requestDeleteToken(e, t) {
    ...
    try {
        ...
        if (a.error) {
            ...
            throw C.create("token-unsubscribe-failed", { errorInfo: e });
        }
    } catch (e) {
        throw C.create("token-unsubscribe-failed", {
        ...
        if (
            ...
            return Date.now() >= r.createTime + 6048e5
                ? (async function updateToken(e, t) {
                      try {
                          const n = await (async function requestUpdateToken(
                                  e,
                                  t
                              ) {
                                  const n = await getHeaders(e),
                                      r = getBody(t.subscriptionOptions),
                                      a = {
                                          method: "PATCH",
                                          headers: n,
                                  ...
                                  try {
                                      ...
                                      throw C.create("token-update-failed", {
                                          errorInfo:
                                  ...
                                  if (!o.token)
                                      throw C.create("token-update-no-token");
                                  return o.token;
...
async function deleteTokenInternal(e) {
    ...
    t &&
        ...
        await (async function dbRemove(e) {
            const t = getKey(e),
                n = (await getDbPromise()).transaction(E, "readwrite");
...
async function getNewToken(e, t) {
    const n = await (async function requestGetToken(e, t) {
            ...
            try {
                ...
                throw C.create("token-subscribe-failed", {
                    errorInfo: null == e ? void 0 : e.toString(),
                });
            }
            if (o.error) {
                const e = o.error.message;
                throw C.create("token-subscribe-failed", { errorInfo: e });
            }
            if (!o.token) throw C.create("token-subscribe-no-token");
            return o.token;
        })(e, t),
...
function getMissingValueError(e) {
    return C.create("missing-app-config-values", { valueName: e });
}
_mergeStrings(
...
async function registerDefaultSw(e) {
    try {
        ...
        throw C.create("failed-service-worker-registration", {
            browserErrorMessage: null == e ? void 0 : e.message,
        });
    }
}
async function getToken$1(e, t) {
    if (!navigator) throw C.create("only-available-in-window");
    if (
        ("default" === Notification.permission &&
            (await Notification.requestPermission()),
        "granted" !== Notification.permission)
    )
        throw C.create("permission-blocked");
    return (
        await (async function updateVapidKey(e, t) {
            t ? (e.vapidKey = t) : e.vapidKey || (e.vapidKey = I);
        })(e, null == t ? void 0 : t.vapidKey),
        await (async function updateSwReg(e, t) {
            if (
                (t || e.swRegistration || (await registerDefaultSw(e)),
                t || !e.swRegistration)
            ) {
                if (!(t instanceof ServiceWorkerRegistration))
                    throw C.create("invalid-sw-registration");
                e.swRegistration = t;
            }
        ...
        return (
            navigator.serviceWorker.addEventListener("message", (e) =>
                (async function messageEventListener(e, t) {
                    ...
                    e.onMessageHandler &&
                        n.messageType === S.PUSH_RECEIVED &&
                        ("function" == typeof e.onMessageHandler
                            ? e.onMessageHandler(externalizePayload(n))
                            : e.onMessageHandler.next(externalizePayload(n)));
                    const r = n.data;
                    (function isConsoleMessage(e) {
                        return (
                            "object" == typeof e &&
                            !!e &&
                            "google.c.a.c_id" in e
                        );
                    })(r) &&
                        "1" === r["google.c.a.e"] &&
                        (await logToScion(e, n.messageType, r));
                })(t, e)
            ),
            t
        );
...
function getMessagingInWindow(e = n()) {
    return (
        isWindowSupported().then(
            (e) => {
                if (!e) throw C.create("unsupported-browser");
            },
            (e) => {
                throw C.create("indexed-db-unsupported");
            }
        ),
...
function deleteToken(e) {
    return (async function deleteToken$1(e) {
        if (!navigator) throw C.create("only-available-in-window");
        return (
            e.swRegistration || (await registerDefaultSw(e)),
            deleteTokenInternal(e)
        );
...
!(function registerMessagingInWindow() {
    t(new Component("messaging", WindowMessagingFactory, "PUBLIC")),
        t(
            new Component(
                "messaging-internal",
